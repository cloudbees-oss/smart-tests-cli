...
Perhaps your pipeline tests multiple components or plugins, or maybe your framework forces this kind of organization (for example, xref:references:integrations.adoc#dotnet-test[dotnet test] organizes tests by `.dll` ). In any case, all the groups are tested together (comprising a single xref:concepts:test-session.adoc[Test Session]), but each group has its own small test suite.

To better support this scenario, xref:subset/zero-input-subsetting.adoc[Zero Input Subsetting] makes it possible to request a single "high level" subset across all components.
...
For example, we have three components: A, B, and C. Each group has five test items. We'll assign each to groups. (Note the use of `--group` on each `smart-tests record tests` )
...
[NOTE]
--
The examples on this page describe a scenario with only three groups. This is just for illustrative purposes. In reality, this approach is for teams with lots of groups (e.g. 10+).
--
...
. `smart-tests subset` with the `--split` option added. This option modifies the command's output to return a subset ID string instead of the subset contents. You'll use this ID in the next command.
...
. `smart-tests split-subset` with the `--split-by-group` option. This command outputs several files for your pipeline (see below).
...
For more information, refer to the xref:references:cli-reference.adoc[CLI reference] for additional options.
...
{PRODUCT} follows the normal instructions for using an exclusion rule so that only those tests run. For more information, refer to the documentation for your test runner.

For example, here's a basic invocation of xref:references:integrations.adoc#maven[Maven] for component A, complete with test recording at the end:

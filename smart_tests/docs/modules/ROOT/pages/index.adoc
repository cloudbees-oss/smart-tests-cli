...
{PRODUCT} is a *software development intelligence platform* focused on continuous integration (CI). Using data from your CI runs, {PRODUCT} provides various features to speed up your testing workflow so you can ship high-quality software faster.
...
To see aggregate info about your test sessions, including average test session duration, test session frequency, and how often sessions fail refer to xref:features:test-trends-and-insights.adoc[test suite trends].
...
To sign up for a {PRODUCT} account, refer to link:https://cloudbees.io[https://cloudbees.io] and xref:cloudbees-unify:teams-users:users.adoc#signup[sign up]. Once done, you can start xref:send-data-to-smart-tests:send-data-to-smart-tests.adoc[sending data to {PRODUCT}].